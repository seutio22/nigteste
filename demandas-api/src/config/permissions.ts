...
// Chaves alinhadas ao frontend (cadastro = demandas, manutencao, projetos)
...
// Módulo usado na API: cadastro (rotas /demandas), manutencao (/manutencoes), etc.
export type ApiModule = keyof SystemPermissions;
...
/**
 * Retorna as permissões efetivas do usuário: prioriza o JSON salvo (customizado)
 * e faz merge com o padrão do role. Compatível com frontend (cadastro, manutencao, projetos).
 */
export function getUserPermissions(
  ...
  if (!permissionsJson || typeof permissionsJson !== 'string' || !permissionsJson.trim()) {
    return { ...defaults };
  }
  try {
    const parsed = JSON.parse(permissionsJson) as Record<string, ModulePermission>;
    if (!parsed || typeof parsed !== 'object') return { ...defaults };
    const merged = { ...defaults } as SystemPermissions;
    for (const key of Object.keys(parsed) as (keyof SystemPermissions)[]) {
      if (key in merged && parsed[key] && typeof parsed[key] === 'object') {
        merged[key] = { ...defaults[key], ...parsed[key] } as ModulePermission;
      }
    }
    if (!merged.cadastro && (parsed as any).demandas) {
      merged.cadastro = (parsed as any).demandas as ModulePermission;
    }
    if (!merged.demandas && merged.cadastro) {
      merged.demandas = merged.cadastro;
    }
    return merged;
  } catch {
    return { ...defaults };
