...
export async function userRoutes(app: FastifyInstance, options: { prisma: PrismaClient }) {
  ...
  app.get('/users', { preHandler: ensureAdmin }, async () => {
    try {
      ...
      app.log.error('Erro ao listar usuários:', error as any)
  ...
  app.get('/users/:id', { preHandler: ensureAdmin }, async (req: any) => {
    try {
      ...
      app.log.error('Erro ao obter usuário:', error as any)
      throw error
    }
  })
  ...
  app.post('/users', { preHandler: ensureAdmin }, async (req: any, res: any) => {
    try {
      ...
      app.log.error('Erro ao criar usuário:', error as any)
      if (error instanceof z.ZodError) {
        return res.code(400).send({ error: 'Dados inválidos', details: (error as any).errors })
      }
  ...
  app.put('/users/:id', { preHandler: ensureAdmin }, async (req: any, res: any) => {
    try {
      ...
      app.log.error('Erro ao atualizar usuário:', error as any)
      if (error instanceof z.ZodError) {
        return res.code(400).send({ error: 'Dados inválidos', details: (error as any).errors })
      }
  ...
  app.delete('/users/:id', { preHandler: ensureAdmin }, async (req: any) => {
    try {
      ...
      app.log.error('Erro ao deletar usuário:', error as any)
      throw error
    }
  })
  ...
  app.patch('/users/:id/toggle-active', { preHandler: ensureAdmin }, async (req: any) => {
    try {
      ...
      app.log.error('Erro ao alterar status do usuário:', error as any)
      throw error
    }
  })
  ...
  app.get('/users/me', async (req: any) => {
    try {
      ...
      app.log.error('Erro ao obter dados do usuário:', error as any)
      throw error
    }
  })
}
