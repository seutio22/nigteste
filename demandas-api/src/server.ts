...
// Rota para normalizar status de Manutenção
app.post('/migrate/normalize-manutencao-status', async (request: any, reply: any) => {
  try {
    console.log('🔄 Iniciando normalização de status de Manutenção via API...')
    ...
    const variacoesEncontradas: Record<string, number> = {}
    let totalParaNormalizar = 0
    
    for (const variacao of statusToNormalize) {
    ...
    console.log(`\n📊 Variações encontradas para normalizar: ${totalParaNormalizar} registro(s)`)
    console.log(`📊 Status "Concluída" (padrão correto): ${countConcluida} registro(s)`)
    ...
    for (const [variacao, count] of Object.entries(variacoesEncontradas)) {
      ...
      normalizados[variacao] = result.count
    }
    ...
    console.log(`✅ Normalização concluída! Total normalizado: ${totalNormalizado}`)
    console.log(`📈 Total de registros com status "Concluída": ${countFinalConcluida}`)
    
    return {
      success: true,
      message: 'Status de Manutenção normalizados com sucesso',
      count: totalNormalizado,
      normalized: Object.keys(normalizados).join(', ') + ' -> Concluída',
    ...
    console.error('❌ Erro ao normalizar status de Manutenção:', error)
    reply.code(500)
    return {
      success: false,
      error: error.message || 'Erro ao normalizar status de Manutenção'
    }
  }
})
