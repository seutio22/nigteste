...
export function filterContratoResumoPorVisibilidade(
  ...
  const allColunas = resumo.allColunas.filter((c) => !ocultas.has(c.id))
  const filtered = resumo.pages
    .map((p) => ({
      ...p,
      colunas: p.colunas.filter((c) => !ocultas.has(c.id)),
    }))
    .filter((p) => p.colunas.length > 0)

  const total = filtered.length
  const pagesRenumbered = filtered.map((p, pageIndex) => ({
    ...p,
    pageIndex,
    totalPages: total,
  }))
