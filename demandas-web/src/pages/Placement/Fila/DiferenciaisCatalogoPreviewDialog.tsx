    ...
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          {isImport
            ? `Fornecedor: ${fornecedorNome}. Revise os diferenciais encontrados em Dados → Diferenciais antes de preencher o lançamento.`
            : `Fornecedor: ${fornecedorNome}. Confirme os registros que serão gravados em Dados → Diferenciais para reutilização em outras cotações.`}
        </Typography>

        {!rows.length ? (
          <Alert severity="warning">
            {isImport
              ? 'Nenhum diferencial encontrado no catálogo para este fornecedor e planos da proposta.'
        ...
        {skippedCount > 0 && (
          <Alert severity="info" sx={{ mt: 2 }}>
            {skippedCount} linha(s) da cotação não entraram na prévia por falta de plano vinculado em Dados →
            Planos.
          </Alert>
        )}
      </DialogContent>
