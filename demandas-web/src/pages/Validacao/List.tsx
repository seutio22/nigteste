...
import { generateUniqueValidationTicket } from '../../utils/validationTicket'
...
function ActionCell({ id, status }: { id: string, status: string }) {
  ...
  const doDuplicate = async () => {
    ...
    try {
      const newTicket = generateUniqueValidationTicket(
        v.ticket,
        store.items.map((item) => item.ticket)
      )
      ...
      const navigateId = duplicated?.id
      if (!navigateId) throw new Error('Duplicação não retornou ID')
