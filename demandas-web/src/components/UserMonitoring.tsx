  ...
  const loadMonitoringData = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.log(`✅ Encontrados ${monitoringData.length} registros de monitoramento`)
        ...
        const averageTimeToday = monitoringData.reduce((sum, u) => sum + u.totalTimeToday, 0) / totalUsers
        const averageTimeThisMonth = monitoringData.reduce((sum, u) => sum + u.totalTimeThisMonth, 0) / totalUsers
        
        const mostActive = monitoringData.reduce((max, u) => u.totalTimeToday > max.totalTimeToday ? u : max)
        const leastActive = monitoringData.reduce((min, u) => u.totalTimeToday < min.totalTimeToday ? u : min)
        ...
        setStats({
          ...
          mostActiveUser: mostActive.userName,
          leastActiveUser: leastActive.userName,
          totalSessionsToday,
          totalSessionsThisMonth
        })

      } catch (apiError) {
        console.warn('⚠️ API de monitoramento não disponível, usando dados simulados:', apiError)
        
        // Fallback: usar dados simulados baseados em usuários reais
