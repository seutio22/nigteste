...
if (-not $railwaySuccess -or -not $vercelSuccess) {
    Write-Host "Executando fallback via Git..." -ForegroundColor Yellow
    git add .
    git commit -m "trigger: Deploy fallback" --allow-empty
    git push origin main
    Write-Host "Deploy automatico iniciado via Git push" -ForegroundColor Green
}

Write-Host "Deploy finalizado!" -ForegroundColor Magenta
